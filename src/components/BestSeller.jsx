...
const BestSeller = () => {
  ...
  useEffect(() => {
    const rateFilter = data.filter((i) => {
      return i.rating.rate >= 4 && i.category.includes("clothing");
    });
    setFilter(rateFilter);
  }, [data]);
    ...
    <div className="mt-25">
      <div className="text-center">
        <h2 className="text-3xl md:text-5xl uppercase">
          <span className="text-slate-400">Best</span> Seller
        </h2>
        <p>
      ...
      <section className="mt-25 grid gap-3 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {filter.slice(0, 4).map(({ id, image, title, price, rating }) => (
